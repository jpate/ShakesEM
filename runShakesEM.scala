package RunShakesEM {
  ...
  class MostBasicActor extends Actor {
    def receive = {
      case s:String => println( "Got it: " + s )
    }
  }

  object startMostBasicActor {
    def main( args:Array[String] ) {
      import se.scalablesolutions.akka.remote.RemoteNode
      import se.scalablesolutions.akka.remote.RemoteNode._

      RemoteNode.start("localhost",9999)
      RemoteNode.register("basic-service", new MostBasicActor)


    }
  }
  object testMostBasicActor {
    def main( args:Array[String] ) {
      import se.scalablesolutions.akka.remote.{RemoteNode,RemoteClient}
      val actor = RemoteClient.actorFor("basic-service","localhost",9999)
      actor ! "yeehaw"
    }
  }
  ...
  object testRemoteActor {
    def main( args:Array[String] ) {
      ...
      object manager extends ShakesParserManager( initGram ) {
        ...
        def stoppingCondition( iterNum:Int, deltaLogProb:Double ) = true
        def cleanup = { }
      }
      manager.start
    }
